import { Link } from "react-router-dom";
import MenuIcon from '@mui/icons-material/Menu';
import { useState } from "react";

const NAV_ITEMS = [
  { name: "About", hash: "#about" },
  { name: "Projects", hash: "#projects" },
  { name: "Experience", hash: "#experience" },
  { name: "Contact", hash: "#contact" },
];

const NavHamburger = () => {
  const [isOpen, setIsOpen] = useState(false);
  return (
    <div className="">
      {
        isOpen ? (
          <div className="fixed w-screen h-screen flex flex-row z-[100]">
            <NavMenu setIsOpen={setIsOpen} />
            <div className="bg-black bg-opacity-50 z-50 w-1/3" onClick={() => setIsOpen(false)}></div>
          </div>
        ) : (
          <button className="bg-white dark:bg-neutral-700 rounded-full p-3 shadow-lg m-4 fixed top-0 left-0 z-50" onClick={() => setIsOpen(true)}>
            <MenuIcon className="text-3xl dark:text-white text-neutral-400 cursor-pointer" />
          </button>
        )
      }
    </div >
  )
}

const NavMenu = ({ setIsOpen = null }) => {
  return (
    <div className="flex flex-col justify-center dark:text-neutral-400 max-h-screen h-screen bg-white dark:bg-neutral-800 gap-12 absolute left-0 sticky top-0 px-8 lg:w-full w-2/3 z-50 font-semibold shadow-xl">
      <div className="flex flex-col">
        <h1 className="text-3xl font-bold text-main">Tammy Young</h1>
        <p className="text-neutral-500 font-normal dark:text-neutral-400">Software Developer</p>
      </div>
      {
        NAV_ITEMS.map((item, index) => (
          <Link
            key={index}
            to={item.hash}
            className="text-xl hover:text-main transition-colors duration-300 dark:text-neutral-300"
            onClick={() => setIsOpen && setIsOpen(false)}
          >
            {item.name}
          </Link>
        ))
      }
    </div>
  )
}

const Navigation = () => {
  return (
    <>
      <div className="lg:flex hidden lg:w-1/5 lg:min-w-[270px] lg:max-w-[320px]">
        <NavMenu />
      </div>
      <div className="absolute lg:hidden">
        <NavHamburger />
      </div>
    </>
  )
}

export default Navigation;
